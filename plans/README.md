...
| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| [051](051-scan-redirect-guard.md) | Re-validate every redirect hop against the SSRF guard | P1 | M | — | DONE (pure `checkRedirectHop` + a `page.on("response")` listener in `runScan` that resolves each 3xx `Location` against `response.url()` and records the first blocked hop; checked after `await page.goto` and thrown as an ordinary `Error`, so `run-monitor`'s existing catch records it in `lastError`. 5 new tests, 540 passing. Three notes: (1) **the plan's `let blockedHop` shape does not typecheck** — TypeScript keeps a `let`'s initializer narrowing for a variable only ever assigned inside a closure, so `if (blockedHop)` narrows to `never` and the body is unreachable (TS #9998); an array holder (`blockedHops[0]`) sidesteps it without changing the logic, and the reason is commented in place; (2) `response.headers().location` rather than the plan's `["location"]` — Biome's `complexity/useLiteralKeys` flags the computed form, and the return type is an index signature so dot access typechecks; (3) the resolve is wrapped in try/catch — a malformed `Location` is one Chromium cannot follow either, so it is skipped rather than treated as blocked. **The listener sees subresource responses too, not only navigations** — deliberate (an `<img>` that 302s to metadata is the same attack) and low-risk since a direct subresource request to a private address never reaches this listener, but it means one redirecting tracker to a private address fails the whole scan. Untested by automation, per the plan's stated trade. **Not pushed** — the plan says leave it local for review; `MONITOR_SCHEDULER_ENABLED` is now unblocked but stays unset until this ships) |
