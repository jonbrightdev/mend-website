...
| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| [051](051-scan-redirect-guard.md) | Re-validate every redirect hop against the SSRF guard | P1 | M | — | DONE (pure `checkRedirectHop` + a `page.on("response")` listener in `runScan` that resolves each 3xx `Location` against `response.url()` and records the first blocked hop; checked after `await page.goto` and thrown as an ordinary `Error`, so `run-monitor`'s existing catch records it in `lastError`. 10 new tests, 545 passing. Four notes: (1) **the plan's `let blockedHop` shape does not typecheck** — TypeScript keeps a `let`'s initializer narrowing for a variable only ever assigned inside a closure, so `if (blockedHop)` narrows to `never` and the body is unreachable (TS #9998); an array holder (`blockedHops[0]`) sidesteps it without changing the logic, and the reason is commented in place; (2) `response.headers().location` rather than the plan's `["location"]` — Biome's `complexity/useLiteralKeys` flags the computed form, and the return type is an index signature so dot access typechecks; (3) the resolve is wrapped in try/catch — a malformed `Location` is one Chromium cannot follow either, so it is skipped rather than treated as blocked; (4) **the listener filters on `request().isNavigationRequest()`**, matching the plan's prose ("every navigation response") rather than its code shape, which filtered nothing. Blocking subresource redirects too was tried and reverted: a subresource that redirects somewhere private leaks nothing through our output (axe reports on the document, not on what an image fetched), and a hostile page can request an internal address *directly* from its own JavaScript — a path no redirect listener can see — so the check bought a false-positive class (any tracker that 302s fails the whole scan) for no real coverage. A navigation is the case that matters: its DOM becomes the audit, HTML snippets and all. The plan's "listener is untested by automation" trade was **narrowed rather than accepted** — the 3xx filter and the `Location` resolution moved into a pure `blockedRedirect(status, location, responseUrl)` covered by 5 more tests (incl. a relative `Location` on the second hop of a chain, and an `http`→`https` same-site hop still passing, which is one of the plan's STOP conditions); only `isNavigationRequest()` and the push remain browser-only. **Not pushed** — the plan says leave it local for review; `MONITOR_SCHEDULER_ENABLED` is now unblocked but stays unset until this ships) |
...
`src/lib/security-headers.ts` before attempting it); **subresource requests to
private addresses** (surfaced while executing 051 and *not* introduced by it —
`assertScannableUrl` governs which page we navigate to, but a scanned page's own
markup can request `http://169.254.169.254/` directly via `<script src>`/`<img>`,
which no redirect listener observes; the read-back paths are weak, since axe
reports on the document and cross-origin `fetch` cannot read a response without
CORS, but closing it properly means request-level interception — `page.route` —
which is a different design decision from the one 051 made); and the **doxy.me
